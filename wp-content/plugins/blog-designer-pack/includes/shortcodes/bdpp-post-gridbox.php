<?php
/**
 * 'bdp_post_gridbox' Post GridBox Shortcode
 * 
...
/**
 * Function to handle the `bdp_post_gridbox` shortcode
 * 
 * @since 1.0
 */
function bdp_render_post_gridbox( $atts, $content = null ) {
	...
	$shortcode_designs 				= bdp_post_gridbox_designs();
	$atts['shortcode']				= 'bdp_post_gridbox';
	...
	$args = apply_filters( 'bdpp_post_gridbox_query_args', $args, $atts );
	...
	if ( $query->have_posts() ) {
		...
		include( BDP_DIR . '/templates/gridbox/loop-start.php' );
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			$atts['feat_img'] 	= bdp_get_post_feat_image( $post->ID );
			...
			// Include Dsign File
			include( BDP_DIR . "/templates/gridbox/{$atts['design']}.php" );

		endwhile;

		include( BDP_DIR . "/templates/gridbox/loop-end.php" );
	}
...
// Post GridBox Shortcode
add_shortcode( 'bdp_post_gridbox', 'bdp_render_post_gridbox' );