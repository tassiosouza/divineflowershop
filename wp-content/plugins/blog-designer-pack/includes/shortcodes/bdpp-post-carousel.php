<?php
/**
 * 'bdp_post_carousel' Post Carousel Shortcode
 * 
 ...
 * @since 1.0
 */
function bdp_render_post_carousel( $atts, $content = null )  {
	...
	$shortcode_designs 				= bdp_post_carousel_designs();
	...
	$atts['slide_show']				= bdp_clean_number( $atts['slide_show'], 2 );
	$atts['slide_scroll']			= bdp_clean_number( $atts['slide_scroll'], 1 );
	...
	$atts['count'] 			= 0;
	$atts['slider_conf'] 	= array('slide_show' => $atts['slide_show'], 'slide_scroll' => $atts['slide_scroll'], 'loop' => $atts['loop'], 'arrows' => $atts['arrows'], 'dots' => $atts['dots'], 'autoplay' => $atts['autoplay'], 'autoplay_interval' => $atts['autoplay_interval'], 'speed' => $atts['speed']);
	...
	if ( $query->have_posts() ) {
		...
		include( BDP_DIR . '/templates/carousel/loop-start.php' );
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			include( BDP_DIR . "/templates/carousel/{$atts['design']}.php" );

		endwhile;

		include( BDP_DIR . "/templates/carousel/loop-end.php" );
	}
...
add_shortcode( 'bdp_post_carousel', 'bdp_render_post_carousel' );