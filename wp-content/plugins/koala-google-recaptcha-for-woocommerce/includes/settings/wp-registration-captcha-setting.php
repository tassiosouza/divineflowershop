...
if ( ! function_exists( 'wp_regs_captcha_check_setting' ) ) {
	...
	function wp_regs_captcha_check_setting() {
		?>
		<input type="checkbox" name="wp_regs_captcha_check" value="1"<?php checked( get_option( 'wp_regs_captcha_check' ), '1' ); ?>><p><?php echo esc_html__( 'Enables captcha for WordPress registration page', 'recaptcha_verification' ); ?></p>
		<?php
	}
}
if ( ! function_exists( 'wp_regs_captcha_title_set' ) ) {
	...
	function wp_regs_captcha_title_set() {
		?>
		<span class="ka-grc-tooltip" data-tooltip="<?php echo esc_html__( 'Field text for WordPress registration captcha', 'recaptcha_verification' ); ?>"><span class="ka-grc-tooltip-trigger">?</span></span>
		<input type="text" name="wp_regs_captcha_title" value="<?php echo esc_attr( get_option( 'wp_regs_captcha_title' ) ); ?>">
		<?php
	}
}
if ( ! function_exists( 'wp_regs_captcha_themes_settings' ) ) {
	...
	function wp_regs_captcha_themes_settings() {
		?>
		<input type="radio" name="wp_regs_captcha_theme_fields"value="light"<?php checked( get_option( 'wp_regs_captcha_theme_fields' ), 'light' ); ?>  checked >
		<label for="Light"><?php echo esc_html__( 'Light', 'recaptcha_verification' ); ?></label>
		<br>
		<input type="radio" name="wp_regs_captcha_theme_fields" value="dark"<?php checked( get_option( 'wp_regs_captcha_theme_fields' ), 'dark' ); ?>  >
...
if ( ! function_exists( 'wp_regs_captcha_size_setting' ) ) {
	...
	function wp_regs_captcha_size_setting() {
		?>
		<input type="radio" name="wp_regs_add_captcha_size_radio" value="normal"<?php checked( get_option( 'wp_regs_add_captcha_size_radio' ), 'normal' ); ?>  checked  >
		<label for="Light"><?php echo esc_html__( 'Normal', 'recaptcha_verification' ); ?></label>
		<br>
		<input type="radio" name="wp_regs_add_captcha_size_radio"value="compact"<?php checked( get_option( 'wp_regs_add_captcha_size_radio' ), 'compact' ); ?>  >
