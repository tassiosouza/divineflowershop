...
if ( class_exists( 'Iconic_WDS_Core_Telemetry' ) ) {
	return;
}

use Iconic_WDS_NS\StellarWP\Telemetry\Core as Telemetry;
use Iconic_WDS_NS\StellarWP\Telemetry\Config;
use Iconic_WDS_NS\StellarWP\Telemetry\Opt_In\Status;

/**
 * Iconic_WDS_Core_Telemetry.
 *
 * @class    Iconic_WDS_Core_Telemetry
 * @version  1.0.0
 */
class Iconic_WDS_Core_Telemetry {
	/**
	 * Single instance of the Iconic_WDS_Core_Telemetry object.
	 *
	 * @var Iconic_WDS_Core_Telemetry|null
	 */
	...
	/**
	 * Creates/returns the single instance Iconic_WDS_Core_Telemetry object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Iconic_WDS_Core_Telemetry
	 */
	public static function run( $args = array() ) {
		if ( null === self::$single_instance ) {
			$default_args = array(
				'settings_class'           => 'Iconic_WDS_Core_Settings',
	...
	public static function optin_args( $args, $stellar_slug ) {
		...
		// Translators: The plugin name.
			__( 'Opt in to help us enhance %s', 'jckwds' ),
		...
		$args['permissions_url'] = 'https://iconicwp.com/telemetry-tracking/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=opt-in&utm_content=jckwds';
		$args['tos_url']         = 'https://iconicwp.com/terms/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=opt-in&utm_content=jckwds';
	...
	public static function exit_interview_args( $args, $stellar_slug ) {
		...
		$args['uninstall_reasons'][] = [
			...
			'uninstall_reason'    => __( "I'm just deactivating temporarily.", 'jckwds' ),
		];
	...
	public static function optin_setting( $settings = array() ) {
		...
		$settings['sections'][ self::$args['opt_out_settings_section'] ]['fields'][] = array(
			'id'       => 'optin',
			'title'    => __( 'Opt In', 'jckwds' ),
			// Translators: The telemetry terms URL.
			'subtitle' => sprintf( __( 'Help us improve this plugin by sharing <a href="%s" target="_blank">non-sensitive</a> data with us.', 'jckwds' ), 'https://iconicwp.com/telemetry-tracking/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=opt-in-setting&utm_content=jckwds' ),
			'type'     => 'toggle',
			'default'  => $status,
		);
	...
	public static function save_optin_setting() {
		...
		if ( ! wp_verify_nonce( $nonce, self::$args['option_group'] . '-options' ) ) {
			wp_die( esc_html__( 'Sorry, you can\'t do that.', 'jckwds' ) );
		}
