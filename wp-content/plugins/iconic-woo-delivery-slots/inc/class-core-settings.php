...
class Iconic_WDS_Core_Settings {
	...
	public static function support_link() {
		...
		return sprintf(
			...
			esc_url($url),
			esc_html__( 'Submit Ticket', 'jckwds' ) );
	}
	...
	public static function documentation_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			$url,
			esc_html__( 'Read Documentation', 'jckwds' )
		);
	}
	...
	public static function account_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			esc_url($url),
			esc_html__( 'Manage Your Account', 'jckwds' )
		);
	}
	 ...
	 * @return string
	 */
	protected static function add_utm_query_args($url, $campaign, $medium = 'Plugin', $content = 'jckwds', $source = null) {
		$args = [
