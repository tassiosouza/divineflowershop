...
if ( class_exists( 'Class_Prefix_Core_Settings' ) ) {
	return;
}

/**
 * Class_Prefix_Core_Settings.
 *
 * @class    Class_Prefix_Core_Settings
 * @version  1.0.6
 */
class Class_Prefix_Core_Settings {
	/**
	 * Single instance of the Class_Prefix_Core_Settings object.
	 *
	 * @var Class_Prefix_Core_Settings
	 */
	 ...
	 *
	 * @var Class_Prefix_Settings_Framework
	 */
	...
	/**
	 * Creates/returns the single instance Class_Prefix_Core_Settings object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Class_Prefix_Core_Settings
	 */
	...
	private function __construct() {
		...
		if ( defined( 'CLASS_PREFIX_FILE' ) ) {
			add_filter( 'plugin_action_links_' . plugin_basename( CLASS_PREFIX_FILE ), array( __CLASS__, 'plugin_action_links' ) );
		}
	}
	...
	public static function init() {
		...
		self::$settings_framework = new Class_Prefix_Settings_Framework( self::$args['settings_path'], self::$args['option_group'] );
	...
	public static function get_plugin_slug() {
		return str_replace( '-', '_', '%textdomain%' );
	}
	...
	public static function add_settings_page() {
		if ( ! defined( 'ICONIC_DISABLE_BRAND' ) && ! defined( 'CLASS_PREFIX_DISABLE_BRAND' ) ) {
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, '%textdomain%-hide-notice', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	public static function output_getting_started_links() {
		...
		?>
		<h3><?php esc_html_e( 'Getting Started', '%textdomain%' ); ?></h3>
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => esc_html__( 'Dashboard', '%textdomain%' ),
		);

		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'CLASS_PREFIX_DISABLE_DASH' ) ) {
			$settings['sections']['license'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => esc_html__( 'License &amp; Account Settings', '%textdomain%' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						...
						'subtitle' => __( 'Activate your license to get the latest updates and support.', '%textdomain%' ),
						'type'     => 'custom',
						'output'   => '',
					),
					array(
						'id'       => 'account',
						'title'    => __( 'Your Account', '%textdomain%' ),
						'subtitle' => __( 'Manage all of your Iconic plugins, supscriptions, renewals, and more.', '%textdomain%' ),
						'type'     => 'custom',
			...
			$settings['sections']['support'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'support',
				'section_title'       => esc_html__( 'Support', '%textdomain%' ),
				'section_description' => '',
				'section_order'       => 30,
				'fields'              => array(
					array(
						...
						'subtitle' => esc_html__( 'Get premium support with a valid license.', '%textdomain%' ),
						'type'     => 'custom',
					...
					array(
						...
						'subtitle' => esc_html__( 'Read the plugin documentation.', '%textdomain%' ),
						'type'     => 'custom',
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Class_Prefix_Core_Cross_Sells::get_selected_plugins();
		$api_sidebars = Class_Prefix_Core_Cross_Sells::get_api_sidebars();
									...
									<div class="iconic-product__buttons">
										<p>
											<a href="<?php echo esc_url( $checkout_url ); ?>" class="button iconic-button iconic-button--small" target="_blank">
												<?php esc_html_e( 'Buy Plugin', '%textdomain%' ); ?>
											</a>
										</p>
										<p><?php esc_html_e( '30 Day Money-Back Guarantee', '%textdomain%' ); ?></p>
									</div>
								<?php } ?>
							</div>
						</div>
	...
	public static function support_link() {
		...
		return sprintf(
			...
			esc_url($url),
			esc_html__( 'Submit Ticket', '%textdomain%' ) );
	}
	...
	public static function documentation_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			$url,
			esc_html__( 'Read Documentation', '%textdomain%' )
		);
	}
	...
	public static function account_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			esc_url($url),
			esc_html__( 'Manage Your Account', '%textdomain%' )
		);
	}
	...
	public static function plugin_action_links( $links ) {
		...
		$action_links = array(
			'settings' => '<a href="' . admin_url( $settings_path ) . '" aria-label="' . esc_attr__( 'View settings', '%textdomain%' ) . '">' . esc_html__( 'Settings', '%textdomain%' ) . '</a>',
		);
	 ...
	 * @return string
	 */
	protected static function add_utm_query_args($url, $campaign, $medium = 'Plugin', $content = '%textdomain%', $source = null) {
		$args = [
