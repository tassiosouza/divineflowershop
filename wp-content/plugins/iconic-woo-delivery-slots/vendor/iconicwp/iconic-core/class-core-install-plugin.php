 ...
 * Settings class.
 */
class Class_Prefix_Core_Install_Plugin {
	/**
	...
	public function __construct( $slug, $plugin_path, $settings_url = false, $source_plugin_settings_url = false ) {
		...
		$this->source_plugin_slug         = '%textdomain%';
	...
	public function display_error_notices() {
		...
		if ( $install_error ) {
			...
			<div class="notice notice-error is-dismissible">
				<p><?php esc_html_e( 'There was an error installing the plugin. Please try again or install manually.', '%textdomain%' ); ?></p>
			</div>
			<?php
		}
		...
		if ( $activate_error ) {
			?>
			<div class="notice notice-error is-dismissible">
				<p><?php esc_html_e( 'There was an error activating the plugin. Please try again or activate manually.', '%textdomain%' ); ?></p>
			</div>
			<?php
		}
	}
	...
	public function output_button( $args = array() ) {
		$defaults = array(
			'text'              => __( 'Install', '%textdomain%' ),
			'class'             => 'button',
			'show_manage_link'  => false,
			'activate_text'     => __( 'Activate', '%textdomain%' ),
			'manage_link_text'  => __( 'Manage', '%textdomain%' ),
		...
		if ( is_plugin_active( $this->plugin_path ) && ! empty( $args['show_manage_link'] ) && ! empty( $args['manage_link_text'] ) ) {
			...
			<div class="iconic-plugin-installed" style="display:flex; gap:2px;">
				...
				<div>
					<div><?php esc_html_e( 'Enabled', '%textdomain%' ); ?></div>
					<a 
				...
				$( '.<?php echo esc_js( $unique_id ); ?> a' ).click( function( e ) {
					...
					$.ajax( {
						...
						error: function( response ) {
							iconicShowErrorNotice( $container.parent(), '<?php esc_html_e( 'There was an error installing the plugin. Please try again or install manually.', '%textdomain%' ); ?>' );
						}
					} );
				} );
			} );
	...
	public function install_plugin_ajax() {
		...
		if ( ! $plugin ) {
			wp_send_json_error( __( 'No plugin specified', '%textdomain%' ) );
		}

		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( __( 'You do not have permission to install plugins', '%textdomain%' ) );
		}
		...
		wp_send_json_success(
			array(
				'message'      => __( 'Plugin installed successfully', '%textdomain%' ),
