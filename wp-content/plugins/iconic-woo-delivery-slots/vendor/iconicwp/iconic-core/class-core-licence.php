...
if ( class_exists( 'Class_Prefix_Core_Licence' ) ) {
	return;
}

/**
 * Class_Prefix_Core_Licence.
 *
 * @class    Class_Prefix_Core_Licence
 * @version  1.0.0
 */
class Class_Prefix_Core_Licence {
	/**
	 * Single instance of the Class_Prefix_Core_Licence object.
	 *
	 * @var Class_Prefix_Core_Licence
	 */
	...
	/**
	 * Creates/returns the single instance Class_Prefix_Core_Licence object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Class_Prefix_Core_Licence
	 */
	...
	public static function connect_before() {
		...
		if ( class_exists( 'Class_Prefix_Core_Settings' ) ) {
			Class_Prefix_Core_Settings::$settings_framework->settings_header();
		}
	}
	...
	public static function back_to_settings_link( $html = '' ) {
		return $html . sprintf( '<a href="%s" class="button button-secondary">&larr; %s</a>', self::$args['urls']['settings'], esc_html__( 'Back to Settings', '%textdomain%' ) );
	}
	...
	public static function output_back_to_settings_link() {
		if ( ! Class_Prefix_Core_Settings::is_settings_page( '-account' ) ) {
			return;
		}
	...
	public static function add_action_links( $links ) {
		$links[] = sprintf( '<a href="%s" target="_blank">%s</a>', self::$args['urls']['product'] . '/changelog/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=changelog-link&utm_content=%textdomain%', esc_html__( 'Changelog', '%textdomain%' ) );
	...
	public static function admin_account_link() {
		return sprintf( '<a href="%s" class="button button-secondary">%s</a>', self::$args['urls']['account'], esc_html__( 'Manage Licence &amp; Billing', '%textdomain%' ) );
	}
