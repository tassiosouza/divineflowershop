...
if ( class_exists( 'Class_Prefix_Core_License_Uplink' ) ) {
	return;
}

use Class_Prefix_NS\StellarWP\Uplink\Admin\License_Field;
use Class_Prefix_NS\StellarWP\Uplink\Config;
use Class_Prefix_NS\StellarWP\Uplink\Uplink;
use Class_Prefix_NS\StellarWP\Uplink\Register;

/**
 * Class_Prefix_Core_License_Uplink.
 *
 * @class    Class_Prefix_Core_License_Uplink
 * @version  1.0.0
 */
class Class_Prefix_Core_License_Uplink {
	/**
	 * Single instance of the Class_Prefix_Core_License_Uplink object.
	 *
	 * @var Class_Prefix_Core_License_Uplink|null
	 */
	...
	/**
	 * Creates/returns the single instance Class_Prefix_Core_License_Uplink object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Class_Prefix_Core_License_Uplink
	 */
	...
	private function __construct() {
		...
		add_filter( 'stellarwp/uplink/' . self::$args['plugin_slug'] . '/messages/expired_key_link', array( __CLASS__, 'expired_key_link' ), 10, 1 );
	}
	...
	public static function dequeue_scripts_and_styles() {
		...
		if ( ! Class_Prefix_Core_Settings::is_settings_page() ) {
			return;
		}
	...
	public static function add_action_links( $links ) {
		$links[] = sprintf( '<a href="%s" target="_blank">%s</a>', esc_url( self::$args['urls']['product'] ) . '/changelog/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=changelog-link&utm_content=%textdomain%', __( 'Changelog', '%textdomain%' ) );
	 ...
	 * Expired key link
	 *
	 * @param string $link   Link to renew the license.
	 *
	 * @return string
	 */
	public static function expired_key_link( $link ) {
		return 'https://my.iconicwp.com';
	}
}
