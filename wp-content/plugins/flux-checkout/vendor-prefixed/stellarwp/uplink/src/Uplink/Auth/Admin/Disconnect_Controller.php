...
use Iconic_Flux_NS\StellarWP\Uplink\Auth\Token\Disconnector;
...
final class Disconnect_Controller {
	...
	public function maybe_disconnect(): void {
		...
		if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), self::ARG ) ) {
			if ( $this->disconnect->disconnect() ) {
				$this->notice->add(
					new Notice( Notice::SUCCESS,
						__( 'Token disconnected.', 'flux-checkout' ),
						true
					)
				);
			} else {
				$this->notice->add(
					new Notice( Notice::ERROR,
						__( 'Unable to disconnect token, ensure you have admin permissions.', 'flux-checkout' ),
						true
					)
				);
			}
			...
			$this->notice->add(
				new Notice( Notice::ERROR,
					__( 'Unable to disconnect token: nonce verification failed.', 'flux-checkout' ),
					true
				)
			);
		}
