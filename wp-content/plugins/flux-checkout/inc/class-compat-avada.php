...
class Iconic_Flux_Compat_Avada {
	...
	public static function compat_avada() {
		if ( ! class_exists( 'Avada_Woocommerce' ) || ! Iconic_Flux_Core::is_flux_template() ) {
			return;
		}
		...
		if ( function_exists( 'AWB_Off_Canvas_Front_End' ) ) {
	...
	public static function compat_avada_disable_css() {
		if ( ( ! Iconic_Flux_Core::is_checkout() && ! Iconic_Flux_Core::is_thankyou_page() ) || ! class_exists( 'Fusion_Dynamic_CSS' ) ) {
			return;
		}
	 ...
	 * Custom checkout CSS.
	 */
	public static function custom_checkout_css() {
		...
		?>
		<style>
			.flux-checkout__order-review .fusion-product-name-wrapper .product-thumbnail {
				display: none;
			}
		</style>
		<?php
