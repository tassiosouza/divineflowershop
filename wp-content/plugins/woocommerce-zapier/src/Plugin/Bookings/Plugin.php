...
class Plugin extends Base {
	...
	public function initialise() {
		...
		add_action(
			...
			function ( $transitioned_from, $transitioned_to, $booking_id ) {
				...
				/**
				 * Execute the Zapier Integration handler for woocommerce_booking_status_changed.
				 *
				...
				if ( 'in-cart' === $transitioned_from && 'cancelled' !== $transitioned_to ) {
					// Booking ordered.
					/**
					 * Execute the Zapier Integration handler for woocommerce_booking_ordered.
		...
		add_action(
			...
			function ( $booking_id ) {
				...
				if ( get_wc_booking( $booking_id ) ) {
					/**
					 * Execute the Zapier Integration handler for wp_trash_post.
		...
		add_action(
			...
			function ( $booking_id, $data ) {
				...
				/**
				 * Execute the Zapier Integration handler for pre_post_update.
				 *
				 * @param int $booking_id Booking ID.
				 *
				 * @since 2.2.0
		...
		add_action(
			...
			function ( $booking_id ) {
				...
				if ( get_wc_booking( $booking_id ) ) {
					/**
					 * Execute the Zapier Integration handler for untrashed_post.
					 * Run for booking only.
					 *
					 * @param int $booking_id Booking ID.
					 *
	...
	/**
	 * Remove Bookings endpoints that are not required by Zapier Integration, including:
	 *
