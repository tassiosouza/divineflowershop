...
class Resources {
	 ...
	 *
	 * @since 2.2.0
	 * @since 2.14.0 Changed the return type to always be an array even if the payload is a WP_Error.
	 *
	...
	public function woocommerce_webhook_payload( $payload, $resource_key, $resource_id, $webhook_id ) {
		...
		// Convert a WP_Error to an array.
		// This occurs for resources that aren't built in to WooCommerce core.
		if ( is_wp_error( $payload ) ) {
			$payload = \rest_convert_error_to_response( $payload );
			return (array) $payload->get_data();
		}
		return $payload;
	}
