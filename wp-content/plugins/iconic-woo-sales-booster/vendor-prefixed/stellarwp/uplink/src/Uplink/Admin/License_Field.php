...
class License_Field extends Field {
		...
		foreach ( $this->get_resources() as $resource ) {
			...
			add_settings_field(
				...
				[
					...
					'placeholder'  => __( 'License Number', 'iconic-wsb' ),
	...
	public function get_field_html( $plugin ) : string {
		$html = sprintf( '<p class="tooltip description">%s</p>', __( 'A valid license key is required for support and updates', 'iconic-wsb' ) );
