...
if ( class_exists( 'Iconic_WSB_Core_Settings' ) ) {
	return;
}

/**
 * Iconic_WSB_Core_Settings.
 *
 * @class    Iconic_WSB_Core_Settings
 * @version  1.0.6
 */
class Iconic_WSB_Core_Settings {
	/**
	 * Single instance of the Iconic_WSB_Core_Settings object.
	 *
	 * @var Iconic_WSB_Core_Settings
	 */
	 ...
	 *
	 * @var Iconic_WSB_Settings_Framework
	 */
	...
	/**
	 * Creates/returns the single instance Iconic_WSB_Core_Settings object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Iconic_WSB_Core_Settings
	 */
	...
	private function __construct() {
		...
		if ( defined( 'ICONIC_WSB_FILE' ) ) {
			add_filter( 'plugin_action_links_' . plugin_basename( ICONIC_WSB_FILE ), array( __CLASS__, 'plugin_action_links' ) );
		}
	}
	...
	public static function init() {
		...
		self::$settings_framework = new Iconic_WSB_Settings_Framework( self::$args['settings_path'], self::$args['option_group'] );
	...
	public static function get_plugin_slug() {
		return str_replace( '-', '_', 'iconic-wsb' );
	}
	...
	public static function add_settings_page() {
		if ( ! defined( 'ICONIC_DISABLE_BRAND' ) && ! defined( 'ICONIC_WSB_DISABLE_BRAND' ) ) {
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, 'iconic-wsb-hide-notice', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	public static function output_getting_started_links() {
		...
		?>
		<h3><?php esc_html_e( 'Getting Started', 'iconic-wsb' ); ?></h3>
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => esc_html__( 'Dashboard', 'iconic-wsb' ),
		);

		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'ICONIC_WSB_DISABLE_DASH' ) ) {
			$settings['sections']['license'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => esc_html__( 'License &amp; Account Settings', 'iconic-wsb' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						'id'       => 'license_field',
						'title'    => __( 'License Key', 'iconic-wsb' ),
						'subtitle' => __( 'Activate your license to get the latest updates and support.', 'iconic-wsb' ),
						'type'     => 'custom',
						'output'   => '',
					),
					array(
						'id'       => 'account',
						'title'    => __( 'Your Account', 'iconic-wsb' ),
						'subtitle' => __( 'Manage all of your Iconic plugins, supscriptions, renewals, and more.', 'iconic-wsb' ),
						'type'     => 'custom',
			...
			$settings['sections']['support'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'support',
				'section_title'       => esc_html__( 'Support', 'iconic-wsb' ),
				'section_description' => '',
				'section_order'       => 30,
				'fields'              => array(
					array(
						'id'       => 'support',
						'title'    => esc_html__( 'Support', 'iconic-wsb' ),
						'subtitle' => esc_html__( 'Get premium support with a valid license.', 'iconic-wsb' ),
						'type'     => 'custom',
						'output'   => self::support_link(),
					),
					array(
						'id'       => 'documentation',
						'title'    => esc_html__( 'Documentation', 'iconic-wsb' ),
						'subtitle' => esc_html__( 'Read the plugin documentation.', 'iconic-wsb' ),
						'type'     => 'custom',
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Iconic_WSB_Core_Cross_Sells::get_selected_plugins();
		$api_sidebars = Iconic_WSB_Core_Cross_Sells::get_api_sidebars();
		...
		<div class="iconic-settings-sidebar">
			...
			if ( ! empty( $cross_sells ) ) {
				...
				<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--works-well">
					<h3><?php esc_html_e( 'Works well with...', 'iconic-wsb' ); ?></h3>
									...
									<div class="iconic-product__buttons">
										<p>
											<a href="<?php echo esc_url( $checkout_url ); ?>" class="button iconic-button iconic-button--small" target="_blank">
												<?php esc_html_e( 'Buy Plugin', 'iconic-wsb' ); ?>
											</a>
										</p>
										<p><?php esc_html_e( '30 Day Money-Back Guarantee', 'iconic-wsb' ); ?></p>
									</div>
								<?php } ?>
							</div>
						</div>
	...
	public static function support_link() {
		...
		return sprintf(
			...
			esc_url($url),
			esc_html__( 'Submit Ticket', 'iconic-wsb' ) );
	}
	...
	public static function documentation_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			$url,
			esc_html__( 'Read Documentation', 'iconic-wsb' )
		);
	}
	...
	public static function account_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			esc_url($url),
			esc_html__( 'Manage Your Account', 'iconic-wsb' )
		);
	}
	...
	public static function plugin_action_links( $links ) {
		...
		$action_links = array(
			'settings' => '<a href="' . admin_url( $settings_path ) . '" aria-label="' . esc_attr__( 'View settings', 'iconic-wsb' ) . '">' . esc_html__( 'Settings', 'iconic-wsb' ) . '</a>',
		);
	 ...
	 * @return string
	 */
	protected static function add_utm_query_args($url, $campaign, $medium = 'Plugin', $content = 'iconic-wsb', $source = null) {
		$args = [
