...
function elessi_enqueue_scripts() {
    ...
    if (isset($nasa_opt['show_notification_leave_tab']) && $nasa_opt['show_notification_leave_tab']) {
        $notification = isset($nasa_opt['notification_leave_tab']) && $nasa_opt['notification_leave_tab'] != '' ?
            $nasa_opt['notification_leave_tab'] : "😁 Come back! | ️‍🔥 Don't forget this ...";
    ...
    if (NASA_WOO_ACTIVED) {
        ...
        if ((!isset($nasa_opt['disable-quickview']) || !$nasa_opt['disable-quickview'])) {
            ...
            if (
                (isset($nasa_opt['loop_layout_buttons']) && $nasa_opt['loop_layout_buttons'] == 'modern-8') ||
                $change_variation_mini_cart
            ) {
                wp_enqueue_script('wc-add-to-cart-variation');
